...
from listedness_service import dataframe_to_index, assess, render_listedness_updater
init_shared_context()
import pandas as pd
...
for _p in _all_products:
    _name = clean_value(_p.get("Ingredient", "")) or clean_value(_p.get("Drug Name", "")) or clean_value(_p.get("Drug", ""))
    if _name and _name not in _ingredients: _ingredients.append(_name)
_pts = []
for _e in _all_events:
    _pt = clean_value(_e.get("PT", "")) or clean_value(_e.get("Event Term", ""))
    if _pt and _pt not in _pts: _pts.append(_pt)
_rows=[]
for _ingredient in _ingredients:
    for _pt in _pts:
        _exp,_comment=assess(listedness_index,_ingredient,_pt)
        _rows.append({"Active Ingredients":_ingredient,"PT":_pt,"Expectedness":_exp,"Comment":_comment})
if _rows:
    st.dataframe(pd.DataFrame(_rows),use_container_width=True,hide_index=True)
else:
    st.info("No product/event PT combinations are available for listedness assessment.")
render_listedness_updater("quality_listedness", _ingredients, _pts)
